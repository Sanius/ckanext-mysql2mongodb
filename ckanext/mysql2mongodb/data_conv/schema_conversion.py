...
from ckanext.mysql2mongodb.data_conv.utilities import extract_dict, import_json_to_mongodb, open_connection_mongodb, \
    open_connection_mysql, drop_mongodb_database, load_mongodb_collection, store_json_to_mongodb
...
class SchemaConversion:
    ...
    def __save(self):
        ...
        # print("Ready
        import_json_to_mongodb(db_connection, collection_name="schema", dbname=self.schema_conv_output_option.dbname,
                               json_filename=self.schema_filename)
    ...
    def get_tables_name_list(self):
        ...
        table_name_list = list(
            map(lambda table: table["name"], list(filter(lambda table: table["remarks"] == "", self.tables_schema))))
    ...
    def data_type_schema_mapping(self, mysql_type):
        ...
        # dtype_dict["blob"] = []
        dtype_dict["string"] = ["JSON", "CHARACTER", "CHARSET", "ASCII", "UNICODE", "CHAR", "VARCHAR", "TINYTEXT",
                                "TEXT", "MEDIUMTEXT", "LONGTEXT", "GEOMETRY", "POINT", "LINESTRING", "POLYGON",
                    ...
                    if table["name"] in idx_table_name_type_dict:
                        ...
                        if index_type == "BTREE":
                            if num_sub_index == 1:
                                ...
                                collection.create_index([(col_dict[idx_uuid], 1) for idx_uuid in index_cols],
                                                        unique=index_unique)
            ...
            for index_schema in table_schema["indexes"]:
                if type(index_schema) is dict:
                    index_column_list = list(
                        map(lambda col_sche: {"name": col_sche["name"], "table": col_sche["short-name"].split(".")[0]},
                            list(
                                filter(lambda col_sche: col_sche["@uuid"] in index_schema["columns"], columns_schema))))
                    index_info = {
