...
from ckanext.mysql2mongodb.data_conv.utilities import open_connection_mysql, open_connection_mongodb, \
    import_json_to_mongodb, extract_dict, store_json_to_mongodb, load_mongodb_collection
...
class DataConversion(AbstractDataConversion):
    ...
    def create_one_table(self, mysql_connection, table_info):
        try:
            ...
            primary_key_info = list(filter(
                lambda index_info: index_info["uuid"] == table_info["primary-key-uuid"],
                self.get_primary_indexes_info_list()))[0]
    ...
    def migrate_one_collection_to_table(self, mysql_connection, collection_name):
        ...
        try:
            ...
            table_coluuid_list = list(filter(
                lambda table_schema: table_schema["name"] == collection_name, db_schema["catalog"]["tables"]))[0][
                "columns"]
    ...
    def find_converted_dtype(self, mysql_dtype):
        ...
        dtype_dict[mongodb_dtype["string"]] = ["CHARACTER", "CHARSET", "ASCII",
                                               "UNICODE", "CHAR", "VARCHAR", "TINYTEXT", "TEXT", "MEDIUMTEXT",
                                               "LONGTEXT"]
            ...
            for i in range(len(col_fetch_seq)):
                ...
                if cell_data != None:
                    ...
                    # print(cell_data, type(cell_data))
                        ...
                        # print(cell_data, type(cell_data))
                        converted_data = datetime(cell_data.year, cell_data.month,
                                                  cell_data.day)  # , cell_data.hour, cell_data.minute, cell_data.second)
                    # elif dtype == "JSON":
            ...
            for key in tables_relations:
                if tables_relations[key]["primary_key_table"] == original_table:
                    ...
                    edited_table_relations_dict[original_table] = edited_table_relations_dict[original_table] + [
                        extract_dict(["primary_key_column", "foreign_key_table", "foreign_key_column"])(
                            tables_relations[key])]
        ...
        for original_collection_name in tables_name_list:
            if original_collection_name in original_tables_set:
                for relation_detail in edited_table_relations_dict[original_collection_name]:
                    ...
                    self.convert_one_relation_to_reference(original_collection_name, referencing_collection_name,
                                                           original_key, referencing_key)
        print("Convert relations successfully!")

    def convert_one_relation_to_reference(self, original_collection_name, referencing_collection_name, original_key,
                                          referencing_key):
        """
        ...
        for key in new_referenced_key_dict:
            ...
            referencing_documents.update_many({referencing_key: key},
                                              update={"$set": {referencing_key_new_name: new_reference}})
